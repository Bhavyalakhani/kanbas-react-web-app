...
function QuizAttemptResultsScreen() {
  ...
  const getQuizData = async () => {
    ...
    try {
      ...
      const attemptData = await quizClient.getAttemptsForUserAndQuiz(
        userId,
        qid
      ); // Assume this API fetches user attempt data
      ...
      quizData.questions.forEach((question: any) => {
        if (question.type === "multipleChoice") {
          ...
          initialCorrectAnswers[question._id] =
          ...
          initialCorrectAnswers[question._id] = question.isTrue
            ? "true"
            : "false";
      ...
      console.log(
        "correct answers from useeffect:" +
          JSON.stringify(initialCorrectAnswers)
      );
      ...
      let totalScore = 0;
      quizData.questions.forEach((question: any) => {
        const ans = attemptData.answers.find(
  ...
  const getQuestionResult = (questionId: string) => {
    console.log(
      "Correct answers from state variable: " + JSON.stringify(correctAnswers)
    );
    ...
    if (userAnswer && correctAnswers[questionId].includes("::")) {
      if (correctAnswers[questionId].includes(userAnswer.answer))
  ...
  return (
    <div className="container mt-4">
      <h1 className="fs-3 fw-bold text-primary mb-4">
        {quiz.title} - Attempt Results
      </h1>
      ...
      {quiz.questions.map((question: any) => {
        ...
        return (
          QuestionComponent && (
            <div
              key={question._id}
              className={`card mb-4 rounded p-3 ${
                isCorrect
      ...
      <div className="mt-4">
        <h5 className="fs-5 text-black">
          Attempt Timestamp: {new Date(timestamp).toLocaleString()}
        </h5>
      </div>
    </div>
  );
